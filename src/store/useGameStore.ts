                    ...
                    if (foundIndex !== undefined && foundIndex !== -1) {
                        ...
                        if (autocomplete && value === BoardIcon.CHECK) {
                            _.set(
                                gameBoard,
                                `[${foundIndex}].values`,
                                gameBoard[foundIndex].values.map((v) => ({ ...v, icon: BoardIcon.CROSS }))
                            )
                        }
                        _.set(gameBoard, `[${foundIndex}].values[${playerIndex}]`, value)
                        _.set(gameBoard, `[${foundIndex}].badges[${playerIndex}]`, badge)
                        return set({ gameBoard, ts: Date.now() })
                    }
                }
                return set({ ts: Date.now() })
